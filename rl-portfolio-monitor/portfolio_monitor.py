...
from portfolio_loader import load_portfolio_config, get_holding_by_code, get_all_portfolios
...
def _build_portfolio_kpi_html(name: str, perf_data: list[dict], alerts: list[dict]) -> str:
    """构建单个组合的 KPI 区块 HTML"""
    total_contribution = sum(
        (d["position_pct"] / 100) * d["change_pct"]
        for d in perf_data if d.get("has_data")
    )
    data_rows = [d for d in perf_data if d.get("has_data")]
    total_up = sum(1 for d in data_rows if d["change_pct"] > 0)
    total_down = sum(1 for d in data_rows if d["change_pct"] < 0)
    total_flat = len(data_rows) - total_up - total_down
    contrib_color = "var(--red)" if total_contribution >= 0 else "var(--green)"
    return f"""
  <div class="kpi">
    <div class="label">持仓数量</div>
    <div class="value">{len(perf_data)}</div>
  </div>
  <div class="kpi">
    <div class="label">上涨</div>
    <div class="value up">▲ {total_up}</div>
  ...
  <div class="kpi">
    <div class="label">触发告警</div>
    <div class="value {'red' if alerts else ''}">{len(alerts)}</div>
  </div>
  <div class="kpi">
    <div class="label">今日回报</div>
    <div class="value {'red' if total_contribution >= 0 else 'green'}"
         style="color:{contrib_color}">{total_contribution:+.3f}%</div>
  </div>
""", total_contribution


def _build_table_rows_html(all_perf_data: list[dict], triggered_alerts: list[dict]):
    """构建持仓明细表行 HTML（供单组合和多组合共用）"""
    rows = []
    total_contribution = 0.0
    for d in all_perf_data:
        has_data = d.get("has_data", False)
    ...
    for a in triggered_alerts:
        code = a.get("code", "")
        alerts_by_code.setdefault(code, []).append(a)
    ...
    alert_rows = ""
    for a in triggered_alerts:
        badge = alert_badge_map.get(a.get("type", ""), f'<span class="badge-neutral">{a.get("type","")}</span>')
        trigger = a.get("trigger_value", "")
        headline = a.get("headline", "")
        alert_rows += f"""
        <tr>
    ...
    return perf_rows_html, alert_rows, total_contribution, len(data_rows)


def generate_multi_portfolio_html(
    portfolio_results: dict,
    output_path: str,
) -> str:
    """
    生成多组合持仓监控 HTML 报告，通过 Tab 切换。
    portfolio_results: { "润铭": {"perf_data": [...], "alerts": [...]}, "CIF": {...}, ... }
    """
    from datetime import datetime
    ...
    # ── 头部 KPI：所有组合并排 ──
    all_kpi_html = ""
    contribs = {}  # portfolio_name -> total_contribution
    ...
    for i, name in enumerate(portfolio_names):
        ...
        tabs_html += f'<input type="radio" name="portfolio-tab" id="tab-{i}" class="tab-input" {checked}>'

        perf_rows, alert_rows, total_contrib, data_count = _build_table_rows_html(
            perf_data, alerts
        )

        contrib_color = "var(--red)" if total_contrib >= 0 else "var(--green)"

        alert_section = ""
        if alerts:
            alert_section = f"""
<div class="card">
  <h2>触发告警（{len(alerts)} 条）</h2>
  <table>
    <thead>
...
<div class="card">
  ...
  <div class="no-alerts">✅ 今日无告警</div>
        ...
        tab_content_html += f"""
        <div class="tab-panel" id="panel-{i}">
          <div class="contrib-total">
            {name} 今日估算涨跌幅：
            <span style="color:{contrib_color}">{total_contrib:+.3f}%</span>
            <span class="contrib-note">（{data_count}/{len(perf_data)} 只有效数据）</span>
          </div>
          <div class="card">
            <h2>持仓明细（按持仓占比排序，点击列头可重排）</h2>
            <table>
              <thead>
                <tr>
                  <th>代码</th>
                  <th>公司名称</th>
                  <th class="num sortable" onclick="sortTable({i}, 2, this)">持仓占比<span class="sort-arrow" data-dir="desc">▼</span></th>
                  <th class="num sortable" onclick="sortTable({i}, 3, this)">今日涨跌幅<span class="sort-arrow" data-dir="desc">▼</span></th>
                  <th class="num sortable" onclick="sortTable({i}, 4, this)">组合贡献<span class="sort-arrow" data-dir="desc">▼</span></th>
                </tr>
              </thead>
              <tbody>
                {perf_rows}
    ...
    tabs_label_html = "\n".join(
        f'<label class="tab-label" for="tab-{i}">{name}</label>'
        for i, name in enumerate(portfolio_names)
    )
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  th.sortable:hover {{ color: var(--text); }}
  th.sortable .sort-arrow {{ position: absolute; right: 6px; top: 50%; transform: translateY(-50%); font-size: 0.7rem; }}
  td {{ padding: 9px 12px; border-bottom: 1px solid var(--border); }}
  tr:last-child td {{ border-bottom: none; }}
  tr:hover td {{ background: rgba(255,255,255,0.03); }}
  .num {{ font-family: var(--mono); text-align: right; }}
  ...
  .no-alerts {{ color: var(--text-muted); text-align: center; padding: 24px; }}
  .footer {{ text-align: center; color: var(--text-muted); font-size: 0.75rem; margin-top: 24px; }}
  .contrib-total {{ font-family: var(--mono); font-size: 1.1rem; font-weight: 700; padding: 12px 16px; background: rgba(59,130,246,0.1); border-radius: 8px; margin-bottom: 16px; display: flex; align-items: center; gap: 12px; }}
  .contrib-note {{ font-size: 0.75rem; color: var(--text-muted); font-weight: 400; }}
  /* Tab 切换 */
  .tab-input {{ display: none; }}
  .tab-labels {{ display: flex; gap: 0; margin-bottom: 20px; border-bottom: 2px solid var(--border); }}
  .tab-label {{
    padding: 10px 24px; cursor: pointer; font-weight: 600;
    color: var(--text-muted); font-size: 0.9rem; border-bottom: 2px solid transparent;
    margin-bottom: -2px; transition: all 0.2s;
  }}
  .tab-label:hover {{ color: var(--text); }}
  .tab-panel {{ display: none; }}
  #tab-0:checked ~ .tab-labels label[for="tab-0"],
  ...
  #tab-0:checked ~ .tab-panels #panel-0,
...
<h1>📊 持仓监控日报</h1>
<p class="subtitle">{date_str} · 数据来源：同花顺 iFinD</p>
...
<div class="footer">
  由 rl-portfolio-monitor 生成 · {time_str}
</div>
</div>
<script>
// 排序状态: 每个 Tab 每列的排序方向
...
function sortTable(tabIdx, colIdx, thEl) {{
  const panel = document.getElementById('panel-' + tabIdx);
  const table = panel.querySelector('table');
  const tbody = table.querySelector('tbody');
  const rows = Array.from(tbody.querySelectorAll('tr'));

  if (!sortState[tabIdx]) sortState[tabIdx] = {{}};
  const colState = sortState[tabIdx][colIdx];
  const asc = colState ? !colState.asc : true;
  sortState[tabIdx][colIdx] = {{ asc: asc }};

  const allThs = table.querySelectorAll('th.sortable .sort-arrow');
  allThs.forEach(s => s.textContent = '');
  thEl.querySelector('.sort-arrow').textContent = asc ? '\u25B2' : '\u25BC';
  ...
  rows.sort((a, b) => {{
    ...
    if (isNaN(aNum) || isNaN(bNum)) return aVal.localeCompare(bVal, 'zh');
...
def generate_interactive_multi_html(
    portfolio_results: dict,
    holdings_map: dict,
    output_path: str,
) -> str:
    """
    生成多组合交互式 HTML 报告（带刷新按钮 + Tab 切换）。
    portfolio_results: { "润铭": {"perf_data": [...], "alerts": [...]}, ... }
    holdings_map: { "润铭": [{"code":..., "name":...}, ...], ... } 用于 API 刷新
    """
    from datetime import datetime
...
<div class="card">
  ...
  <div class="no-alerts">✅ 今日无告警</div>
        ...
        <div class="tab-panel" id="panel-{i}">
          <div class="contrib-total">
            {name} 今日估算涨跌幅：
            <span style="color:{contrib_color}" id="contrib-{i}">{total_contrib:+.3f}%</span>
            <span class="contrib-note">（{data_count}/{len(perf_data)} 只有效数据）</span>
          </div>
          <div class="card">
            <h2>持仓明细（点击列头排序）</h2>
            <table data-tab="{i}">
              <thead>
                <tr>
                  <th>代码</th>
                  <th>公司名称</th>
                  <th class="num sortable" onclick="sortTable({i}, 2, this)">持仓占比<span class="sort-arrow">▼</span></th>
                  <th class="num sortable" onclick="sortTable({i}, 3, this)">今日涨跌幅<span class="sort-arrow">▼</span></th>
                  <th class="num sortable" onclick="sortTable({i}, 4, this)">组合贡献<span class="sort-arrow">▼</span></th>
                </tr>
              </thead>
              <tbody id="perf-{i}">
                {perf_rows}
    ...
    tabs_html = "\n".join(
        f'<input type="radio" name="portfolio-tab" id="tab-{i}" class="tab-input" {"checked" if i == 0 else ""}>'
...
<div class="refresh-bar">
  ...
  <button class="btn btn-alert" id="btn-alert" onclick="refreshAllAlerts()">
    <span id="alert-icon">🔔</span> 预警刷新
  </button>
  <button class="btn btn-ann" id="btn-ann" onclick="refreshAnnouncements()">
...
function setBtnLoading(id, loading) {{
  ...
  if (loading) {{
    ...
    btn.disabled = false;
    icon.textContent = id === 'price' ? '📈' : id === 'alert' ? '🔔' : '📋';
  }}
}}
...
function formatChange(pct) {{
  ...
  const cls = v >= 0 ? 'badge-red-light' : 'badge-green-light';
...
function formatContrib(pct, posPct) {{
  ...
  return `${{contrib >= 0 ? '+' : ''}}${{contrib.toFixed(3)}}%`;
}}

function sortTable(tabIdx, colIdx, thEl) {{
  const tbody = document.getElementById('perf-' + tabIdx);
  const rows = Array.from(tbody.querySelectorAll('tr'));
  if (!sortState[tabIdx]) sortState[tabIdx] = {{}};
  const colState = sortState[tabIdx][colIdx];
  const asc = colState ? !colState.asc : true;
  sortState[tabIdx][colIdx] = {{ asc: asc }};
  const allThs = tbody.closest('table').querySelectorAll('th.sortable .sort-arrow');
  allThs.forEach(s => s.textContent = '');
  thEl.querySelector('.sort-arrow').textContent = asc ? '▲' : '▼';
  rows.sort((a, b) => {{
    const aVal = a.children[colIdx].textContent.trim();
  ...
  try {{
    ...
    for (const d of (data.data || [])) {{
      for (let i = 0; i < Object.keys(HOLDINGS).length; i++) {{
        const tabName = Object.keys(HOLDINGS)[i];
        if (HOLDINGS[tabName].some(h => h.code === d.code)) {{
          if (!byTab[i]) byTab[i] = [];
          byTab[i].push(d);
        }}
      }}
    }}
    for (const [tabIdx, items] of Object.entries(byTab)) {{
      updatePerfTable(parseInt(tabIdx), items);
    }}
    updateRefreshTime();
    showToast(`✅ 股价刷新完成：${{data.ok || 0}}/${{data.count || 0}}`);
  }} catch (e) {{
    showToast('❌ 刷新失败：' + e.message);
  }}
  setBtnLoading('price', false);
}}

async function refreshAllAlerts() {{
  setBtnLoading('alert', true);
  showToast('🔔 正在刷新预警...');
  try {{
    const resp = await fetch('/api/refresh_all');
    const data = await resp.json();
    showToast('🔔 预警刷新已启动，请稍候...');
    // 全量刷新在后台进行，30 秒后自动刷新页面
    setTimeout(() => location.reload(), 30000);
  }} catch (e) {{
    showToast('❌ 刷新失败：' + e.message);
  }}
  setBtnLoading('alert', false);
}}

async function refreshAnnouncements() {{
  setBtnLoading('ann', true);
  showToast('📋 正在刷新公告...');
  try {{
    const resp = await fetch('/api/refresh_announcements');
    const data = await resp.json();
    showToast(`✅ 公告刷新完成：${{data.count || 0}} 条`);
    updateRefreshTime();
    // 刷新后自动重载以更新 badge
  ...
  setBtnLoading('ann', false);
}}

function updatePerfTable(tabIdx, items) {{
  const tbody = document.getElementById('perf-' + tabIdx);
  if (!tbody) return;
  // 构建 code -> data 映射
  const map = {{}};
  for (const d of items) map[d.code] = d;
  // 更新现有行
  for (const row of tbody.querySelectorAll('tr')) {{
    ...
    if (cells.length >= 5) {{
      cells[3].innerHTML = formatChange(d.has_data ? d.change_pct : null);
      cells[4].textContent = formatContrib(d.has_data ? d.change_pct : null, d.position_pct);
    }}
    // 更新行样式
    row.className = d.has_data
      ? (d.change_pct >= 5 ? 'row-red' : d.change_pct >= 0 ? 'row-red-light' :
         d.change_pct <= -5 ? 'row-green' : d.change_pct <= -3 ? 'row-green-light' : 'row-green-light')
      : 'row-na';
  }}
  // 更新组合贡献
  let totalContrib = 0;
  let dataCount = 0;
  const contribEl = document.getElementById('contrib-' + tabIdx);
  for (const d of items) {{
    if (d.has_data) {{
      totalContrib += (d.position_pct / 100) * d.change_pct;
      dataCount++;
    }}
  }}
  if (contribEl) {{
    contribEl.textContent = `${{totalContrib >= 0 ? '+' : ''}}${{totalContrib.toFixed(3)}}%`;
    contribEl.style.color = totalContrib >= 0 ? 'var(--red)' : 'var(--green)';
    const noteEl = contribEl.parentElement.querySelector('.contrib-note');
    if (noteEl) {{
      const totalItems = items.length || tbody.querySelectorAll('tr').length;
      noteEl.textContent = `（${{dataCount}}/${{totalItems}} 只有效数据）`;
    }}
  }}
}}
</script>
...
def generate_interactive_html(
    ...
    for r in sorted_rows:
        anns = ann_by_code.get(r["code"], [])
        if anns:
            for a in anns[:3]:  # 每只最多3条
                badge = get_announcement_badge(a["tag"])
                all_ann_rows.append(
                    f'<tr data-code="{r["code"]}">'
                    f'<td>{r["name"]}</td>'
                    f'<td>{a.get("date", "")}</td>'
                    f'<td>{a["title"][:50]}</td>'
                    f'<td class="num">{badge}</td>'
                    f'<td style="color:var(--text-muted);font-size:0.8rem">{a["comment"]}</td>'
                    f'</tr>'
                )
    ann_rows_html = "\n".join(all_ann_rows) if all_ann_rows else '<div class="no-alerts">暂无新公告</div>'
...
function formatChange(pct) {{
  if (pct === null || pct === undefined) return '<span class="badge-na">N/A</span>';
  const v = parseFloat(pct);
  let cls = v >= 0 ? 'badge-red-light' : 'badge-green-light';
  return `<span class="${{cls}}">${{v >= 0 ? '+' : ''}}${{v.toFixed(2)}}%</span>`;
}}
...
def _normalize_code(code: str) -> str:
    """标准化港股代码：去掉前导零（如 01801.HK → 1801.HK）"""
    if ".HK" in code.upper():
        parts = code.split(".")
...
def _fetch_price_sina(code: str) -> dict:
    """通过新浪财经免费API获取实时行情（ifind备用方案）"""
    try:
        import re
        ...
        if market in ("SZ", "SH"):
            # A股: 名称, 昨收, 开盘, 现价, 最高, 最低, 现价, 成交量, 成交额...
            ...
            # 港股: 英文名, 中文名, 开盘, 昨收, 最高, 最低, 现价, 涨跌额, 涨跌幅(小数)...
    ...
    try:
        ...
        for i, r in enumerate(all_rows):
            ...
            if r.startswith("|证券代码"):
                ...
                for j, col in enumerate(cells):
                    if "前收盘价" in col:
                        prev_price_idx = j
                    elif "收盘价" in col and price_idx is None:
                        price_idx = j
                    elif "涨跌幅" in col:
                        change_idx = j
                data_start = i + 1
        ...
        for r in all_rows[data_start:]:
            ...
            if all(c in ("", "---") for c in cells):
                continue
            if r.startswith("|") and len(cells) > 1:
                data_rows.append(cells)

        if len(data_rows) == 0:
            # 无数据行，尝试备用查询
            return _fetch_performance_fallback(code)

        try:
            row = data_rows[0]
            # 优先使用直接返回的涨跌幅
            if change_idx is not None and len(row) > change_idx:
                change_pct = float(row[change_idx])
                # 如果有收盘价就用收盘价
                if price_idx is not None and len(row) > price_idx and row[price_idx]:
                    today_price = float(row[price_idx])
                elif prev_price_idx is not None and len(row) > prev_price_idx and row[prev_price_idx]:
                    today_price = float(row[prev_price_idx]) * (1 + change_pct / 100)
                else:
                    today_price = 0
                return {
                    "price": round(today_price, 2) if today_price > 0 else 0,
                    "change_pct": round(change_pct, 4),
                    "volume_ratio": 0,
                }
            # 其次用前收盘价+收盘价计算
            elif prev_price_idx is not None and price_idx is not None and len(row) > max(prev_price_idx, price_idx):
                today_price = float(row[price_idx])
                yesterday_price = float(row[prev_price_idx])
...
def _fetch_performance_fallback(code: str) -> dict:
    """备用方案：查询近3日收盘价计算涨跌幅"""
    norm_code = _normalize_code(code)
    result = call_ifind_api(
        "stock",
    ...
    try:
        ...
        try:
            if prev_price_idx is not None and len(data_rows[0]) > prev_price_idx:
                today_price = float(data_rows[0][price_idx])
                yesterday_price = float(data_rows[0][prev_price_idx])
                if today_price <= 0 or yesterday_price <= 0:
                    return {}
                change_pct = (today_price - yesterday_price) / yesterday_price * 100
        ...
        # ifind 全部失败，使用新浪免费 API
        sina_result = _fetch_price_sina(code)
        if sina_result:
            return sina_result
        # 新浪也失败，使用东方财富妙想
        return _fetch_price_miaoxiang(code)


def _fetch_price_miaoxiang(code: str, name: str = "") -> dict:
    """通过东方财富妙想 API 获取行情数据（第 4 层 fallback）"""
    try:
        import subprocess
        query = f"{name} {code} 今日涨跌幅 最新价 昨收" if name else f"{code} 今日涨跌幅 最新价 昨收"
        mx_script = os.path.expanduser("~/Research/ifind mcp&skill/miaoxiang/mx_api.py")
        result = subprocess.run(
            ["python3", mx_script, "--type", "data", "--query", query],
            capture_output=True, text=True, timeout=30
        )
        if result.returncode != 0:
            return {}
        data = json.loads(result.stdout)
        if "error" in data or "tables" not in data:
            return {}
        # 从表格中提取指标数据
        for table in data.get("tables", []):
            indicators = table.get("indicators", {})
            # 尝试匹配涨跌幅相关字段
            price_val = None
            change_val = None
            yesterday_val = None
            for label, values in indicators.items():
                if any(k in label for k in ["收盘价", "最新价", "现价", "收盘"]):
                    val_list = values if isinstance(values, list) else [values]
                    price_val = float(val_list[-1]) if val_list and val_list[-1] else None
                elif any(k in label for k in ["涨跌幅", "涨跌"]):
                    val_list = values if isinstance(values, list) else [values]
                    raw = val_list[-1].rstrip("%") if val_list and val_list[-1] else "0"
                    change_val = float(raw)
                elif any(k in label for k in ["前收"]):
                    val_list = values if isinstance(values, list) else [values]
                    yesterday_val = float(val_list[-1]) if val_list and val_list[-1] else None

            if price_val and change_val is not None:
                return {
                    "price": round(price_val, 2),
                    "change_pct": round(change_val, 4),
                    "volume_ratio": 0,
                }
        return {}
    except Exception:
...
def fetch_announcements(code: str, name: str, days: int = 7) -> list:
    """获取近 N 天新公告列表

    Args:
        code: 股票代码
        name: 公司名称（用作搜索关键词，比代码匹配更准）
        days: 搜索天数范围（默认7天）
    """
    from datetime import datetime, timedelta
    end = datetime.now().strftime("%Y-%m-%d")
    start = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d")
    # 只用公司名称搜索，语义搜索对纯名称匹配更准
    result = call_ifind_api("news", "search_notice", name, time_start=start, time_end=end, size=10)
    try:
        inner_data = result.get("data", {}).get("result", {}).get("content", [{}])[0].get("text", "{}")
        inner = json.loads(inner_data)
...
def scan_all_announcements(days_range: int = 1) -> list[dict]:
    """扫描所有持仓股票的公告，返回情感分析结果

    Args:
        days_range: 日期范围（前后各几天，默认1即昨天+今天+明天）

    Returns:
        [{"code", "name", "title", "date", "sentiment", "tag", "comment"}, ...]
    """
    from datetime import datetime, timedelta
    cutoff_start = (datetime.now() - timedelta(days=days_range)).strftime("%Y-%m-%d")
    cutoff_end = (datetime.now() + timedelta(days=days_range)).strftime("%Y-%m-%d")
    cs = cutoff_start.replace("-", "")
    ce = cutoff_end.replace("-", "")
    try:
    ...
    for holding in holdings:
        ...
        for ann in announcements:
            ...
            if not date:
                continue
            # 过滤日期：仅保留在 cutoff_start 和 cutoff_end 之间
            d_clean = date.replace("-", "")
            if not (cs <= d_clean <= ce):
                continue
...
def save_announcements_to_vault(code: str, name: str, announcements: list):
    ...
    try:
        ...
        first_char = lazy_pinyin(name)[0][0].upper()
        if not first_char.isalpha():
            first_char = "#"
    except Exception:
        first_char = name[0].upper() if name[0].isascii() and name[0].isalpha() else "#"
...
def run_scan_for_portfolio(
    portfolio_name: str,
    portfolio_path: str,
    specific_code: Optional[str] = None,
    push: bool = True,
) -> tuple[list[dict], list[dict]]:
    """对单个组合执行持仓扫描。Returns: (triggered_alerts, all_perf_data)"""
    try:
        holdings = load_portfolio_config(portfolio_path)
    ...
    for holding in holdings:
        ...
        for alert in alerts:
            alert_id = insert_alert(
                code=code, name=name, alert_type=alert["type"],
            ...
            alert["code"] = code
            alert["name"] = name
            alert["portfolio"] = portfolio_name
            triggered_alerts.append(alert)

            if push:
                msg = format_single_alert(
                    name=name, code=code, alert_type=alert["type"],
                    trigger_value=alert["trigger_value"], threshold_value=alert["threshold_value"],
                    headline=alert.get("headline", ""),
    ...
    if all_perf_data:
        ...
        for d in all_perf_data:
            if d.get("has_data"):
                ...
                contribution = 0.0
            change = d["change_pct"]
            flag = " \U0001f7e2" if change <= -3 else (" \U0001f534" if change >= 3 else "")
            rows.append({
                "name": d["name"], "code": d["code"],
                "position_pct": d["position_pct"], "change_pct": change,
                "contribution": contribution, "flag": flag,
                "has_data": d.get("has_data", False),
            })
        rows.sort(key=lambda x: x["position_pct"], reverse=True)
        for r in rows:
            if r["has_data"]:
                ...
                change_str = "N/A"
                contrib_str = "N/A"
            print(f"    {r['name']:<12} {r['code']:<12} {r['position_pct']:>6.2f}%  "
                  f"{change_str:>10}{r['flag']}  {contrib_str:>10}")
        print(f"  {'组合今日估算涨跌幅':>30} {total_contribution:>+.3f}%")
        data_count = sum(1 for d in all_perf_data if d.get("has_data"))
        print(f"  共 {len(all_perf_data)} 只持仓（{data_count} 只有效数据） | 告警 {len(triggered_alerts)} 条")
...
def run_scan(
    ...
    """
    执行持仓扫描，支持单个或多个组合。
    portfolio_name=None -> 扫描所有可用组合
    portfolio_name="CIF" -> 只扫描 CIF 组合
    Returns: {"组合名": {"alerts": [...], "perf_data": [...]}, ...}
    """
    if portfolio_name:
        all_portfolios = get_all_portfolios()
        matched = [p for p in all_portfolios if p["name"] == portfolio_name]
        if not matched:
            print(f"错误：未找到组合 '{portfolio_name}'，可用组合：{[p['name'] for p in get_all_portfolios()]}")
            return {}
        portfolios_to_scan = matched
    else:
        portfolios_to_scan = get_all_portfolios()

    if not portfolios_to_scan:
        print("错误：未找到任何持仓配置文件（*.md）")
        return {}

    print(f"将扫描以下组合：{[p['name'] for p in portfolios_to_scan]}")
    print("=" * 70)

    portfolio_results = {}
    for p in portfolios_to_scan:
        name = p["name"]
        path = p["path"]
        print(f"\n>>> 正在扫描组合：{name}")
        print("-" * 70)
        alerts, perf_data = run_scan_for_portfolio(name, path, specific_code, push)
        portfolio_results[name] = {"alerts": alerts, "perf_data": perf_data}
...
def main():
    ...
    parser.add_argument("--portfolio", default=None, help="指定组合名称（如 CIF），不指定则扫描所有组合")
    ...
    if args.action == "scan":
        ...
        if args.html:
            ...
            os.makedirs(os.path.expanduser("~/.claude/skills/rl-portfolio-monitor/reports"), exist_ok=True)

            if len(portfolio_results) == 1 and portfolio_name:
                # 单组合：使用原有 HTML 格式
                name = list(portfolio_results.keys())[0]
                result = portfolio_results[name]
                suffix = f"_{name}"
                html_path = os.path.expanduser(
                    f"~/.claude/skills/rl-portfolio-monitor/reports/portfolio_report_{today}{suffix}.html"
                )
                generated_path = generate_html_report(result["perf_data"], result["alerts"], html_path)
            else:
                # 多组合：使用 Tab HTML
                suffix = f"_{'_'.join(portfolio_results.keys())}"
                html_path = os.path.expanduser(
                    f"~/.claude/skills/rl-portfolio-monitor/reports/portfolio_report_{today}{suffix}.html"
                )
                generated_path = generate_multi_portfolio_html(portfolio_results, html_path)
