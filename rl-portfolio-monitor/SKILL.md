...
description: 持仓监控告警编排层（支持多组合）。当用户说"持仓监控"、"我的持仓告警"、"扫描持仓"时使用。核心功能：加载 Obsidian 持仓配置 → 多组合同时扫描 → 4 层数据源获取行情 → 独立判断 7 类告警 → 写入 SQLite → 推送飞书。
metadata:
    version: 2.0
...
# RL-Portfolio-Monitor — 持仓监控（多组合版）

## 架构

```
持仓配置 (Obsidian markdown 表格)
     ↓
portfolio_monitor.py   ← 主编排脚本（Python）
  ├─ portfolio_loader.py   ← 加载持仓配置（自动扫描所有组合）
  ├─ alert_history.py      ← SQLite 持久化
  ├─ feishu_formatter.py   ← 飞书消息格式
  ├─ portfolio_server.py   ← 交互式 HTTP 服务（多组合 Tab）
...
自动扫描 `~/Research/Vault_基金经理Agent/` 目录下所有 `.md` 文件，每个文件视为一个组合。

**配置格式**：Markdown 表格，三列结构（列名不限，只要包含 Ticker/名称/权重即可）：
...
解析器自动跳过表头行和 `---` 分隔线，统一按列 1, 2, 3 读取。

**交互式服务**（HTTP 页面）：
- Tab 切换不同组合（类似 Excel Sheet）
- 头部同时显示所有组合的今日回报 KPI
- 三个全局刷新按钮：📈 股价刷新 / 🔔 预警刷新 / 📋 公告刷新
- 每 Tab 独立列头排序（持仓占比 / 涨跌幅 / 组合贡献）
...
**指定组合扫描**：
```
python3 ~/.claude/skills/rl-portfolio-monitor/portfolio_monitor.py scan --portfolio CIF
```
...
# 报告路径：~/.claude/skills/rl-portfolio-monitor/reports/portfolio_report_YYYY-MM-DD_组合1_组合2.html
```
...
**启动本地 HTTP 服务**：
```
python3 ~/.claude/skills/rl-portfolio-monitor/portfolio_server.py --port 8765
```
默认端口 8765，访问 `http://127.0.0.1:8765` 打开交互式页面。

**自定义端口**：
```
python3 ~/.claude/skills/rl-portfolio-monitor/portfolio_server.py --port 9000
```
