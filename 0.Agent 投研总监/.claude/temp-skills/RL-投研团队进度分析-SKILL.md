...
for cdir in company_dirs:
    ...
    # 检查 Wiki 页面
    wiki_base = "/Users/zhuang225/Research/Vault_共享知识库/wiki/companies/"
    has_wiki = False
    for letter_dir in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        wiki_path = os.path.join(wiki_base, letter_dir)
        if os.path.isdir(wiki_path):
            for wf in os.listdir(wiki_path):
                if wf.startswith(comp_name) or (code and code.split('.')[0] in wf):
                    has_wiki = True
                    break
        if has_wiki:
            break
...
| 标签 | 判定条件 |
|------|----------|
| Wiki | 在 `wiki/companies/` 下存在对应公司页面 |
