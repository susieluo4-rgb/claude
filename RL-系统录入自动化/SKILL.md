---
name: RL-系统录入自动化 v1.2
description: 将 ESG / Industry / Porter / QL&RF 四个 TXT 分析报告自动填入打分模板 Excel 的自动化脚本。覆盖分数提取、评论文字写入、精度规则、绿色格子处理。
metadata:
  version: "1.3"
  last_updated: "2026-04-01"  # v1.3: add Market Size L column comments for rows 171-174
...
**ESG维度（行7-44）**：从 ESG.txt 中提取 `**评估理由**：`、`**理由**：`、`**结论**：` 块，写入对应英文 label 行的 L 列。

**行业评分行（行71-130）**：通过 `ind_comments` 和 `port_comments` 映射，将 Industry.txt 和 Porter.txt 中各指标的评分标准说明文字写入 L 列：

```python
# Industry: ind_metric_comment() 提取每个block中metric_kw后的评分说明
ind_comments[(ind_idx, 'cycle')]     # Cycle Score 评分标准
ind_comments[(ind_idx, 'growth')]    # Growth Score 评分标准
ind_comments[(ind_idx, 'scalable')]   # Expansion Score 评分标准
ind_comments[(ind_idx, 'transparency')] # Transparency Score 评分标准

# Porter: port_force_comment() 提取每个block中force后的详细分析
port_comments[(ind_idx, 'competition')]   # Competition 分析
port_comments[(ind_idx, 'supplier')]    # Supplier 分析
port_comments[(ind_idx, 'customer')]     # Customer 分析
port_comments[(ind_idx, 'entry_barrier')]  # Entry Barrier 分析
port_comments[(ind_idx, 'substitution')]   # Substitution 分析
```

**市场规模评论（行171-174）**：`extract_mkt_size_comment()` 从 Industry.txt 各 block 中提取"国内市场规模"和"海外市场规模"说明，格式为：`国内市场规模：2024年约15亿元人民币；海外市场规模：2024年全球约5.5亿美元`。Ind5（行175）不存在于 TXT，跳过。

**QL英文label行（行133-176）**：通过 `ql_en_comment_map` 映射，将 QL&RF.txt 各 section 的分析评论文字写入 L 列。`ql_section_comment()` 函数先用 `blk()` 提取每个 section 块，再在块内搜索关键字，避免全局 first-match 问题：

```python
def ql_section_comment(ql, section_num, next_section='$$', start_kw='**分析：**'):
    block = ql_section_block(ql, section_num, next_section)  # blk() per section
    if block is None: return None
    pos = block.find(start_kw)  # 在块内搜索，不是全局
    ...

ql_en_comment_map = {
    'Score of Visibility':       ql_section_comment(ql, '一', '二', '**评估理由：**'),
    'Margin vs. peers':          ql_section_comment(ql, '二', '三'),
    'marketing & ads expense ratio': ql_section_comment(ql, '三', '四'),
    'number of R&D personnel':   ql_section_comment(ql, '四', '五'),
    'number of patents':         ql_section_comment(ql, '五', '六'),
    'Turnover vs. peers':        ql_section_comment(ql, '六', '七'),
    'Channel control':           ql_section_comment(ql, '七', '八'),
    'Leverage vs. peers':        ql_section_comment(ql, '八', '九'),
}
```

**行177-251**：通过 `get_ql_comment()` 模糊匹配 `ql_comment_map`，将 QL&RF.txt 中的风险穿透评论文字写入 L 列。未匹配到则留空。

**行66 ticker**：`002428.SZ` 写入 I 列。

**ql_i_map**：以下行同步写入 I 列数值：
