...
**保存路径**：`/Users/zhuang225/Research/Vault_公司基本面Agent/11_公司列表/{拼音首字母}/{公司}_{代码}/`
该路径与年报/半年报/季报/研报存放在同一公司文件夹下，便于统一管理。
若文件夹不存在则自动创建。
...
**⚠️ 关键格式说明**：`get_stock_financials` 的 query 参数是**自然语言字符串**，不是结构化参数！

```javascript
// ✅ 正确格式：自然语言query
get_stock_financials({
  query: "宁德时代 300750.SZ 2020-2025年 营业收入 净利润 毛利率 ROE 每股收益"
})
...
```javascript
// 利润表 + 盈利能力
get_stock_financials({ query: "{公司名} {代码} 2020-2025年 营业收入 营业利润 净利润 归母净利润 毛利率 净利率 营业利润率 EBITDA 每股收益" })

// 资产负债表
get_stock_financials({ query: "{公司名} {代码} 2020-2025年 总资产 总负债 所有者权益 货币资金 应收账款 存货 固定资产 短期借款 长期借款 归母权益" })

// 现金流量表
get_stock_financials({ query: "{公司名} {代码} 2020-2025年 经营活动现金流 投资活动现金流 筹资活动现金流 自由现金流 Capex" })

// 估值数据
get_stock_financials({ query: "{公司名} {代码} 市盈率 市净率 市销率 EV EV/EBITDA" })

// 一致预期（最高优先级用于预测部分）
get_stock_financials({ query: "{公司名} {代码} 一致预期 营收FY1 营收FY2 营收FY3 净利FY1 净利FY2 净利FY3 每股收益FY1 毛利率FY1" })

// 行业分类
get_stock_financials({ query: "{公司名} {代码} 申万行业一级 申万行业二级" })
```

**季度历史数据（近12个季度）：**
```javascript
// 使用自然语言query获取季度数据
get_stock_financials({ query: "{公司名} {代码} 2022-2025年各季度 营业收入 净利润 毛利率" })
// 或指定日期范围
get_stock_financials({ query: "{公司名} {代码} 2024Q1-2025Q4 营业收入 归母净利润" })
```
...
**强制步骤：运行验证脚本**
```bash
python3 ~/.claude/skills/rl-company-research-model/scripts/verify_model.py <模型路径>
```
验证脚本会检查 10 项常见问题：
1. 分节标题行是否为空（不能有公式）
2. 财务费用率是否为百分比格式
3. 跨 Sheet 引用正确性（营收增长率、毛利率→业务拆分）
4. 利润表预测列公式完整性
5. 财务费用公式是否引用模型假设
6. 业务拆分预测列格式
7. 资产负债表验证公式
8. 现金流量表期末现金公式
9. 摘要页跨 Sheet 引用
10. **模型假设列引用一致性**（2029E/2030E 是否误引用了 2028E 的列）
...
### 6.3 保存并提供给用户
保存至 `/Users/zhuang225/Research/Vault_公司基本面Agent/11_公司列表/{拼音首字母}/{公司}_{代码}/` 文件夹下，提供 computer:// 链接。
...
16. 公式重算 & 质量检查
17. **运行 verify_model.py 验证脚本，确保全部通过**
18. 保存并提交用户
...
### 问题八：2029E/2030E 列引用了错误的模型假设列（阜博集团实战 Bug）

**现象**：利润表/资产负债表/现金流量表中，2029E 和 2030E 列的公式全部引用了 `模型假设!$D$X`（2028E 的列），而非各自的 `$E$` 和 `$F$`。这是因为 AI 在写公式时复制了上一列的模板，只改了行号没改列号。

**影响**：2029E 和 2030E 使用了与 2028E 完全相同的假设值（增速、毛利率、费用率等），导致预测数据在后两年"卡住"不变。

**正确做法**：每个预测年的公式必须引用模型假设中**对应的列**：
```python
ASM_COL = {'2026E': 2, '2027E': 3, '2028E': 4, '2029E': 5, '2030E': 6}
# B=2, C=3, D=4, E=5, F=6

# ✅ 正确 — 使用 ac(yr) 函数动态获取列字母
def ac(yr): return gcl(ASM_COL[yr])  # 2026E→B, 2027E→C, ..., 2030E→F

ws_is.cell(row, col).value = f'=...模型假设!${ac(yr)}${ASM_ROW["rev_growth"]}'
# 2026E → $B$5, 2027E → $C$5, 2028E → $D$5, 2029E → $E$5, 2030E → $F$5 ✅

# ❌ 错误 — 手动写死列号或复制粘贴后忘记改列
ws_is.cell(row, col).value = '=...模型假设!$D$5'  # 2029E/2030E 也引用了 $D$！
```

**检查方法**：每次构建完成后，运行验证脚本的"模型假设列引用一致性"检查：
```bash
python3 ~/.claude/skills/rl-company-research-model/scripts/verify_model.py <模型路径>
```
验证脚本会从利润表 revenue 行提取每个预测年引用的模型假设列，检查是否各不相同。

**快速人工检查**：利润表 Row4（营业收入）的公式，2029E 列应包含 `$E$`，2030E 列应包含 `$F$`，不能都是 `$D$`。
...
**Excel 财务模型生成完成后**，自动提取关键数据更新对应公司的 Wiki 页面：

```
触发时机：Excel 模型文件创建完成
数据来源：刚生成的 Excel 模型中的财务数据和估值数据
目标页面：~/Research/Vault_共享知识库/wiki/companies/{首字母}/{公司}_{代码}.md
```

**更新内容**：
1. **财务摘要 Section**：从 Excel 的"利润表" Sheet 提取最新报告期数据
   - 营业收入、归母净利润、ROE、经营性现金流、毛利率
2. **估值水平 Section**：从 Excel 的"估值" Sheet 提取
   - PE(TTM)、PB、总市值
3. **最后更新 Section**：更新为当前日期，触发来源标注为 `rl-company-research-model`
4. **追加 wiki/log.md**：记录变更

**⚠️ 如果 Wiki 页面不存在**：跳过此步，等待 `rl-wiki-ingest` 首次编译。
